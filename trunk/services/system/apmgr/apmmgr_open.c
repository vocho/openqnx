...
int apmmgr_open(resmgr_context_t *ctp, io_open_t *msg, void *extra, void *reserved)
{
	switch (msg->connect.subtype)
	{
		...
		case _IO_CONNECT_COMBINE_CLOSE:
		{
			...
			if ((mp = mpath_find((apmmgr_attr_t *)LIST_FIRST(root_mpart->children), part_name, &mp_parent)) == NULL)
			{
							...
							*/
							if ((chk_mode != 0) &&	// ??
								((r = check_access_perms(ctp, (apxmgr_attr_t *)mp_parent, chk_mode, &ci, bool_t_FALSE)) != EOK)) {
									return r;
							} else if ((mp_parent->data.mpid == part_id_t_INVALID) ||
									 (MEMPART_FINDPID(part_pid, mp_parent->data.mpid) == NULL)) {
									return ENOENT;
							}
							else
							{
					...
					*/
					case part_type_ROOT:
					{
						...
						*/
						if (((err = tymem_register(part_name)) != EOK) ||
							((mclass_entry = memclass_find(part_name, memclass_id_t_INVALID)) == NULL))
						{
						...
						mp->hdr.prev = NULL;
						mp->parent = mp_parent;
						mp->data.mclass_entry = mclass_entry;
						mclass_entry->data.rmgr_attr_p = mp;
					...
					case part_type_MEMPART_REAL:
					{
						...
						*/
						...
						if (mp_parent->type == part_type_MEMCLASS)
						{
							memclass_entry_t *mclass = mp_parent->data.mclass_entry;
							mclass_id = mclass->data.info.id;
							CRASHCHECK(memclass_find(NULL, mclass_id) == NULL);
							mpid = part_id_t_INVALID;
						}
						else
						{
							mpid = mp_parent->data.mpid;
							mclass_id = mempart_get_classid(mpid);
						}

						if ((rr = VALIDATE_MP_CFG_CREATION(mpid, NULL, mclass_id)) != EOK)
						{
#ifndef NDEBUG
							if (ker_verbose)
							{
								kprintf("Creation validation err %d, parent (%s), class %p\n",
										rr, mp_parent->name, mclass_id);
							}
#endif	/* NDEBUG */
							PART_ATTR_UNLOCK(mp);
							PART_ATTR_UNLOCK(mp_parent);
						...
						mp->type = part_type_MEMPART_REAL;
						mp->name = strdup(part_name);
						LIST_INIT(mp->children);
						mp->hdr.next = NULL;
						mp->hdr.prev = NULL;
						mp->parent = mp_parent;
						{
							...
							mpart->rmgr_attr_p = (void *)mp;	// back pointer to the resource manager structure
							initial_create = mpart->create_key;
						}
						break;
					}
...
partition_group:
						...
						*/
						...
						mp->hdr.prev = NULL;
						mp->parent = mp_parent;
						mp->data.mpid = part_id_t_INVALID;
						break;
					}
					default:
					{
...
//				iofunc_attr_init(&mp->attr, mp_parent->attr.mode, &mp_parent->attr, &ci);
			...
			r = iofunc_ocb_attach(ctp, msg, &ocb->ocb, &mp->attr, &apmmgr_io_funcs);
