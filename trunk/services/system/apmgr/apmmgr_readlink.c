...
int apmmgr_readlink(resmgr_context_t *ctp, io_readlink_t *msg, RESMGR_HANDLE_T *handle, void *reserved)
{
	...
	if ((mp = mpath_find((apmmgr_attr_t *)LIST_FIRST(root_mpart->children), msg->connect.path, NULL)) == NULL)
		return ENOENT;

	if ((r = PART_ATTR_LOCK(mp)) != EOK)
		return r;

	if (S_ISLNK(mp->attr.mode) && (mp->type == part_type_MEMPART_PSEUDO))
	{
		apmmgr_attr_t *mp_real;
		...
		if ((mp_real = mpath_find((apmmgr_attr_t *)LIST_FIRST(root_mpart->children), link_name, NULL)) == NULL)
			r = ENOLINK;
		else if (mp_real->type != part_type_MEMPART_REAL)
			r = EBADF;
		else
		{
