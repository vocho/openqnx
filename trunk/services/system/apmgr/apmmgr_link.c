 ...
 * apmmgr_link
 * 
 * Provide resource manager symlink processing for the memory partitioning module.
 * This functionality implements the pseudo partition scheme which allows a
 * single name to refer to multiple partitions of differing memory classes so
...
int apmmgr_link(resmgr_context_t *ctp, io_link_t *msg, void *attr, io_link_extra_t *extra)
{
	switch (msg->connect.extra_type)
	{
		case _IO_CONNECT_EXTRA_SYMLINK:
		{
			...
			int  r;
			apmmgr_attr_t  *mp, *mp_parent, *mp_real;
...
//FIX ME - check extra_len first
			...
			if ((mp_real = mpath_find((apmmgr_attr_t *)LIST_FIRST(root_mpart->children), chk_part_name, NULL)) == NULL) {
				return ENOENT;
			}
/*
			mp_parent = NULL;
			if ((mp = mpath_find((apmmgr_attr_t *)LIST_FIRST(root_mpart->children), pseudo_name, &mp_parent)) != NULL)
				return EEXIST;
*/
			...
			while(1)
			{
				...
				mp = mpath_find((apmmgr_attr_t *)LIST_FIRST(mp_parent->children), chk_part_name, NULL);
			...
			if ((mp_parent->type == part_type_MEMPART_REAL) ||
				(mp_parent->type == part_type_MEMPART_PSEUDO))
			{
				PART_ATTR_UNLOCK(mp);
				PART_ATTR_UNLOCK(mp_parent);
				return EACCES;
			...
			if (msg->connect.ioflag & O_CREAT)
			{
				...
				mp->hdr.prev = NULL;
				mp->parent = mp_parent;
				/* use the mpart field point to the mempart_part_t of the real partition */
