...
void *
__dbrealloc(const char *file, int line, void *cptr, size_t size)
{
		void *return_ptr;
    ENTER();
    MALLOC_INIT();
    if (__realloc_hook != NULL) {
    	return_ptr = (*__realloc_hook)(file,line,cptr,size);
    } else {
    	return_ptr = _realloc(cptr,size);
    }
    LEAVE();
		return(return_ptr);
}
...
void *
_realloc_direct(const char *file, int line, void *cptr, size_t size)
{
    return _realloc(cptr,size);
}
...
realloc(void *cptr, size_t size)
{
    ...
    return( __dbrealloc(NULL,line,cptr,size) );
}
