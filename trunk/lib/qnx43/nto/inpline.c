...
#include <ctype.h>
#include <lib43/proto.h>

#define TRUE		(1)
#define FALSE		(0)

#define KEY_UP		0x00a1
#define KEY_DOWN	0x00a9

extern char *input_line();

static int sameCMD( char *s, char *s1 );
static struct history *newBUF( struct history *buf );
...
char *input_line(FILE *fp, unsigned char *lbuf, int bufsize )
/*
 ...
 */
	...
	while( TRUE )
		...
		if( isatty( fileno( fp ) ) )
			...
			n = (fgets((char *)lbuf, bufsize, fp ) == NULL) ? 0 : strlen((char *)lbuf);

		if( n < 1 )
			return( NULL );

		if( isatty(fileno(fp)) && lbuf[0] == 0xff )
			{
			p = NULL;
			if( lbuf[1] == KEY_DOWN )
				p = (h = h->next)->string;
			...
			if( p != NULL )
				{
				updn = 1;
				dev_insert_chars( fileno(fp), strlen(p), p );
				continue;
				}
			}
		...
		if( (lbuf[0] && lbuf[0] != '\f') || lbuf[1] )
			{
			if( strcmp( (char *)lbuf, h->string ) != 0 )
				{
				if( updn == 0 || (! sameCMD(h->string,(char *)lbuf)) )
					{
