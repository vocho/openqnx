...
int posix_spawnattr_getsigdefault(const posix_spawnattr_t *_Restrict attrp, sigset_t *_Restrict sigset_p)
{
	if (!valid_attrp(attrp) || (sigset_p == NULL)) {
		...
		*sigset_p = (_attrp == NULL) ? _default_posix_spawnattr_t.sig.dflt : _attrp->sig.dflt;
		return EOK;
	}
}
...
int posix_spawnattr_getsigmask(const posix_spawnattr_t *_Restrict attrp, sigset_t *_Restrict sigset_p)
{
	if (!valid_attrp(attrp) || (sigset_p == NULL)) {
		...
		_posix_spawnattr_t *_attrp = GET_ATTRP(attrp);
		*sigset_p = (_attrp == NULL) ? _default_posix_spawnattr_t.sig.mask : _attrp->sig.mask;
		return EOK;
	}
}
...
int posix_spawnattr_getsigignore(const posix_spawnattr_t *_Restrict attrp, sigset_t *_Restrict sigset_p)
{
	if (!valid_attrp(attrp) || (sigset_p == NULL)) {
		...
		_posix_spawnattr_t *_attrp = GET_ATTRP(attrp);
		*sigset_p = (_attrp == NULL) ? _default_posix_spawnattr_t.sig.ignore : _attrp->sig.ignore;
		return EOK;
	}
}
