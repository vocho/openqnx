...
{
	if (!valid_attrp(attrp) || (sched_p == NULL)) {
		return EINVAL;
	} else {
		_posix_spawnattr_t *_attrp = GET_ATTRP(attrp);
		*sched_p = (_attrp == NULL) ? _default_posix_spawnattr_t.sched.param : _attrp->sched.param;
		return EOK;
	}
}
...
{
	if (!valid_attrp(attrp) || (policy_p == NULL)) {
		return EINVAL;
	} else {
		_posix_spawnattr_t *_attrp = GET_ATTRP(attrp);
		*policy_p = (_attrp == NULL) ? _default_posix_spawnattr_t.sched.policy : _attrp->sched.policy;
		return EOK;
	}
}
