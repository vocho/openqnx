...
static void rotate(struct block *keylocal, int idx, int dir)
{
	unsigned char *p;
	unsigned char *ep;
	int data0, data28, data27, data55, i;

	if (dir == 0) {
		/* Encrypt: rotate left */
		for (i = rots(idx); i > 0; i--) {
			p = keylocal->b_data;
			ep = &(keylocal->b_data[55]);
			data0 = keylocal->b_data[0];
			data28 = keylocal->b_data[28];
			while (p++ < ep) *(p-1) = *p;
			keylocal->b_data[27] = data0;
			keylocal->b_data[55] = data28;
		}
	}
	else {
		/* Decrypt: rotate right */
		for (i = rots(16 - idx); i > 0; i--) {
			p = &(keylocal->b_data[55]);
			ep = keylocal->b_data;
			data27 = keylocal->b_data[27];
			data55 = keylocal->b_data[55];
			while(p-- > ep) *(p+1) = *p;
			keylocal->b_data[0] = data27;
			keylocal->b_data[28] = data55;
		}
	}
}
...
static void f(struct block *keylocal, struct block *a, struct block *x) {
...
void encrypt(char *blck, int edflag) {
	...
	/* The finale rotate to get the key back to the original state. */
	rotate(&key, i, edflag);
	transpose(p, &swap, 64);
