...
/*
#ifdef __USAGE
%C      - convert a file while copying it.

%C     [if=file] [of=file] [ibs=bytes] [obs=bytes] [bs=bytes] [cbs=bytes]
...
#endif				/* !_POSIX_VERSION */
  copy ();
  return 0; /* will never get here; copy() always exits */
}
  ...
  for (i = 1; i < argc; i++)
    {
      ...
      val = index (name, '=');
      if (val == NULL)
	usage ("unrecognized option `%s'", name, NULL);
      *val++ = '\0';
     ...
     char *str;
{
  ...
  do
	...
	*new++ = '\0';
	...
	{
	  usage ("%s: invalid conversion", str, NULL);
	  exit (1);
	}
      str = new;
  } while (new != NULL);
