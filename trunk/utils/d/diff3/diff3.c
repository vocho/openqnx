...
static char const diff_program[] = "/usr/bin/diff";
...
/* char horizon_arg[17 + INT_STRLEN_BOUND (int)]; */
  ...
  ap = argv;
  *ap++ = diff_program;
/* QNX diff always treats stuff like 8 bit thus it doesn't have this option */
//  if (always_text)
//   *ap++ = "-a";
/* horizon lines help diff optimize output -- QNX diff doesn't support it so we leave it off here */
