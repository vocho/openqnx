...
/*
 * $Log: path.c,v $
 * Revision 1.1  1999/11/30 19:51:26  peterv
 * Added everything...
 *
	...
	char	*path;
{
	...
	if (!*path)
		return;
#if 0	
NOTE: The following chunk of code was added as an effort to make
all our tools mingw.
#ifdef __MSYS__
		/* HACK: we will accept /cygdrive/... paths, by removing leading /cygdrive */
	if (strstr(path, "/cygdrive") == path) {
		cur = path;
		for (t = path + 9; /* 9 == strlen("/cygdrive"); */
				*t; ++t) {
			*cur = *t;
			++cur;
		}
	...
	/* HACK: we convert /c/ to c:/ */
	if (path[0] && ISDIRSEP(path[0]) && path[1] && ISDIRSEP(path[2])) {
		*path = path[1];
		path[1] = ':';
		path[2] = DIRSEP;
	}
	...
	if ((isrooted = ISROOTEDPATH(path)))
		very_start++;
#if defined (OS2) || defined (__CYGWIN__) || defined(__MSYS__)
	if (path[0] && path[1] == ':')	/* skip a: */
		very_start += 2;
#endif /* OS2 || __CYGWIN__ */
#if defined(__QNX__) && !defined(__QNXNTO__)
	if (isrooted && *very_start == '/' && *(very_start+1) != '/')
		very_start++;
#endif /* QNX */
	...
	for (cur = t = start = very_start; ; ) {
		/* treat multiple '/'s as one '/' -- except at start, POSIX */
		while (ISDIRSEP(*t))
			t++;
