...
int main(int argc, char *argv[])
{
	...
	for (cur_comm=_NTO_TRACE_COMM_SMSG;cur_comm<_NTO_TRACE_COMM_SPULSE_QUN;cur_comm++) {
		...
		errno=0;
		rc=TraceEvent(_NTO_TRACE_ADDEVENTHANDLER, _NTO_TRACE_COMM,cur_comm, event_handler, &my_event_data);
		assert(rc!=-1);
		...
		switch (cur_comm) {
			case _NTO_TRACE_COMM_SMSG:
				/* In the smsg case have to guess the results before hand so the
			...
			case _NTO_TRACE_COMM_SPULSE_DEA:
				...
				break;
			case _NTO_TRACE_COMM_SPULSE_UN:
				/* in the case of a pulse, we have to pull out the scoid here for event handlers */
				ConnectServerInfo(getpid(), coid, &info);
				results[0]=info.scoid|_NTO_SIDE_CHANNEL;
				results[1]=info.pid;
				/* We trigger a SMSG and a RMSG the same way */
