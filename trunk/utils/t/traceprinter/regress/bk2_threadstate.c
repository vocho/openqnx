...
/*****************************************************************************
*
*	File:		bk2_threadstate.c
*
******************************************************************************
*
*   Contents:	Tests to make sure the instrumentation properly intercepts
*				thread state changes (scheduler activity) and allows us to
...
int event_handler(event_data_t * event_data)
{
	...
	if ((cur_state) != _NTO_TRACE_GETEVENT(event_data->header))
			correct_values_eh=-1;

	if (data_array[0]!=((cur_state==STATE_STOPPED)?child_pid:mypid)) 
	        correct_values_eh=-2;
	if (data_array[1]!=((cur_state==STATE_STOPPED)?1:exp_thread))
	        correct_values_eh=-3;
