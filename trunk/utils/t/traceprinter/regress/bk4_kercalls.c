...
*				kerel calls, and their arguments on kernel exit and in pass
*				them properly to event handlers
*
*	Date:		August 26, 2001
*
*	Author:		Peter Graves
...
*
*	Purpose: 	This is an kernel call exit handler. It will not really
*				do anything because we don't know the exit values yet.
*
...
{
	if ((correct_values_eh>0) && (cur_event[2]==__KER_TIMER_ALARM))
		my_event_data.data_array=data_array2;
	correct_values_eh++;
	return(1);
}
/****************************************************************************
...
*
*	Purpose: 	This is a function to check that the values passed to
*				the event handler were correct
*
*****************************************************************************/
void check_values(void) 
{
	 ...
	 */
	/* Make sure we get an exit (0-127 are entries, 128-255 are exits, and >255 are interrupted */
	if ((_NTO_TRACE_GETEVENT(my_event_data.header)/128)!=1) 
		correct_values_eh=-2;

	if (_NTO_TRACE_GETEVENT_C(my_event_data.header)!=_TRACE_KER_CALL_C) {
		correct_values_eh=-3;
	}
	if ((_NTO_TRACE_GETEVENT(my_event_data.header)%128)!=cur_event[2]) 
		correct_values_eh=-4;


	if (cur_event[1]!=__KER_THREAD_DESTROYALL) {
	  if(my_event_data.feature[_NTO_TRACE_FIPID]!=mypid)
	    correct_values_eh=-5;
	  if(my_event_data.feature[_NTO_TRACE_FITID]!=mytid)
	    correct_values_eh=-6;

	}
	if (fast_mode==WIDE) {
		if (!((my_event_data.el_num==cur_event[0]) || (my_event_data.el_num==cur_event[1]) ||(my_event_data.el_num==cur_event[0]-1))) {
			correct_values_eh=-1;
			snprintf(buf,sizeof(buf), "Got %d entries, expected %d", 
			  my_event_data.el_num, cur_event[0]);
			testnote(buf);	
			return;
		}
		for (x=0;x<(cur_event[1]);x++) {
			if (my_event_data.data_array[x]!=global_vals[x]) {
				correct_values_eh=-1;
				snprintf(buf,sizeof(buf), "Expected parameter %d to be %d, got %d",
					x, global_vals[x], my_event_data.data_array[x]);
				testnote(buf);
			}
		}
		if (correct_values_eh>0)
			correct_values_eh=1;
		...
		if ((my_event_data.data_array[0]==global_vals[cur_event[4]]) && 
			(cur_event[1]>1)?(my_event_data.data_array[1]==global_vals[cur_event[5]]):1) {
			correct_values_eh=1;
		} else {
			correct_values_eh=-1;
		}
...
//		{4, 1, __KER_MSG_READIOV, (unsigned)"MsgReadiov", 0, 1},
	...
	while (cur_event[0]!=0) {
		...
		rc=TraceEvent(_NTO_TRACE_ADDEVENTHANDLER, _NTO_TRACE_KERCALLEXIT,cur_event[2], event_handler, &my_event_data);
		if (rc==-1)
		...
		rc=TraceEvent(_NTO_TRACE_DELEVENTHANDLER, _NTO_TRACE_KERCALLEXIT, cur_event[2]);
		assert(rc!=-1);
		...
		if (correct_values_eh==0)
			...
			testpntfail("Wrong info passed to event handler");
			}
...
int trigger_kercall_event( unsigned * event_p)
{
	...
	switch(event_p[2]) {
		...
		case __KER_MSG_READIOV:
			...
			assert(coid==COID);
			SETIOV(&reply, reply_buf, 4);
			return_val=MsgSend(coid, "SEND", 5, reply_buf, sizeof(reply_buf));
			rcvid=MsgReceivev(chid, &reply, 1,NULL);
			/* Start logging */
			rc=TraceEvent(_NTO_TRACE_STARTNOSTATE);
			assert(rc!=-1);
			delay(10);
			global_vals[0]=MsgReadiov(rcvid,&reply, 1,0,1);
			if(global_vals[0] == -1){
				testnote(strerror(errno));
				}
			global_vals[1]= ((unsigned *)reply_buf)[0];
