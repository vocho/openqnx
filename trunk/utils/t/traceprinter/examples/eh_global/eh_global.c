...
%C	- example that illustrates the very basic use of
	  ...
	  All events from two classes are included and monitored
	  interchangeably. The flow control of monitoring the
	  specified events is controlled with attached global
	  event handlers.
...
/*
 * Event handler attached to the class _NTO_TRACE_KERCALL
 */
int kercall_eh(event_data_t* e_d)
{
	if(g_state) {
		g_state = !g_state;
		return (1);
...
/*
 * Event handler attached to the class _NTO_TRACE_THREAD
 */
int thread_eh(event_data_t* e_d)
{
	if(!g_state) {
		g_state = !g_state;
		return (1);
...
int main(int argc, char **argv)
{
	...
	/*
	 * Intercept all events from class _NTO_TRACE_KERCALL
	 */
	TRACE_EVENT
	(
	 argv[0],
	...
	/*
	 * Add global event handler to the _NTO_TRACE_KERCALL class
	 */
	TRACE_EVENT
	(
	 argv[0],
	 TraceEvent(_NTO_TRACE_ADDCLASSEVHANDLER, _NTO_TRACE_KERCALL, kercall_eh, &e_d_1)
	);

	/*
	 * Intercept all events from class _NTO_TRACE_THREAD
	 */
	TRACE_EVENT
	(
	 argv[0],
	...
	/*
	 * Add global event handler to the _NTO_TRACE_THREAD class
	 */
	TRACE_EVENT
	(
	 argv[0],
	 TraceEvent(_NTO_TRACE_ADDCLASSEVHANDLER, _NTO_TRACE_THREAD, thread_eh, &e_d_2)
	);
	...
	(
	 argv[0],
	 TraceEvent(_NTO_TRACE_DELCLASSEVHANDLER, _NTO_TRACE_KERCALL)
	);
	TRACE_EVENT
	(
	 argv[0],
	 TraceEvent(_NTO_TRACE_DELCLASSEVHANDLER, _NTO_TRACE_THREAD)
	);
